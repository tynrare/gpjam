...
void main()
{
    ...
    float wavey = cos(uv.y * 60.0 + time * 0.4) * 0.5 + 1.0;
    vec4 noise00 = texture2D(tex_noise0, uv - vec2(0.0 + wavey * 0.1 + sin(time * 0.13), time * 0.1 + wavex * 0.01 ));
    vec4 noise0 = texture2D(tex_noise0, noise00.xy - vec2(wavex * 0.1, time * 0.1));
    vec4 noise1  = texture2D(tex_noise1, uv + noise0.xy * 0.07);

    
    float a = distance(noise1.r * noise0.r, 1.0);
    float alpha = smoothstep(threshold - 0.1, threshold + 0.1, a);

    float rim0 = 1.0 - distance(alpha, 0.8);

    float rim1 = smoothstep(threshold - 0.1, threshold + 0.1, rim0 * alpha);
